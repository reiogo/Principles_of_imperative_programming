...
struct uba_header {
    ...
    int limit; // 0 < limit, NUL not included
...
int uba_len(uba* A){
    REQUIRES(is_uba(A));
    int res = A->size;
    ENSURES(0 <= res && res < (int)strlen(A->data));
    return res;
    }
...
void uba_set(uba* A, int i, char x)
{
    REQUIRES(is_uba(A));
    REQUIRES(0 <= i && i < uba_len(A));
    ENSURES(is_uba(A));
    ...
    /* A may not be a valid array since A->size == A->limit is possible! */
{
    ...
    REQUIRES((int)strlen(A->data) == A->limit);
    char* B = xmalloc(new_limit+1);
    B[new_limit] = '\0';
