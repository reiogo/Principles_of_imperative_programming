...
bool is_ufs(ufs *eqs) {
  ...
  for (int i = 0; i < eqs->size; i++) {
    if (   !(ufs_isin(eqs, -eqs->data[i]))   // -size < i < size
        || !(ufs_isin(eqs, eqs->data[i])))
      return false;
  }
  return true;
}
...
void ufs_union(ufs *eqs, int i, int k) {
  ...
  int hi = -eqs->data[i];      // height of i
  int hk = -eqs->data[k];      // height of k
  if (hi == hk) {          // same height
    eqs->data[i] = k;      //   merge i into k
    eqs->data[k]--;        //   height of k increases
  } else if (hi < hk)      // k is taller
    eqs->data[i] = k;      //   merge i into k
  else {
    ASSERT(hi > hk);       // i is taller
    eqs->data[k] = i;      //   merge k into i
  }
